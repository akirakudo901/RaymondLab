...
def bsoid_frameshift(data_new, fps, clf):
    ...
    labels_fshigh = []
    # for every csv data
    for csv_data in data_new:
        data = data_new[csv_data]
        # create offset versions of the data
        data_offset = []
        # j is the integer floor of how many more bins there can be compared to a 10 fps video
        for j in range(math.floor(fps / 10)):
            data_offset.append(data[j:, :])
        # we extract features from the offset data
        feats_new = bsoid_extract(data_offset)
        # then make predictions on it
        labels = bsoid_predict(feats_new, clf)

        # FOLLOWING SEQUENCE IS PADDING?
        # for each label prediction entry, we reverse their order (NOT SURE WHAT ORDER)?
        for m in range(0, len(labels)): 
            labels[m] = labels[m][::-1]
        # then create a padding which has the dimensions of:
        # [length of labels x ]
