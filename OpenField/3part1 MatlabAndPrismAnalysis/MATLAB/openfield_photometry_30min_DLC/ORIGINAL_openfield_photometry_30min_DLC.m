...
save_dir = "C:\Users\mashi\Desktop\RaymondLab\MATLAB\" + ...
    "open field photometry script\data\315955m1";


file = uigetfile('.csv');
data = readmatrix(file);
graphtitle = file(16:24);
...
hold off
% saveas(gcf,[save_dir,'/Position Graphs/',graphtitle,'.jpg']);
...
disp(totaldistance);
% save([save_dir,'/Total distance pixels/', graphtitle],'totaldistance', '-ascii');
...
% save([save_dir,'/Total diff cm/', graphtitle],'totaldiff_cm', '-ascii');
% save([save_dir,'/Total distance cm/', graphtitle],'totaldistance_cm', '-ascii');
% save([save_dir,'/Total distance cm partial/', graphtitle],'totaldistance_cm', '-ascii');
...
% save([save_dir,'/Center time/', graphtitle],'Centertimesum', '-ascii');
...
if sumOfTime - length(Bellyx)/40 > 0.1
    ...
    % save([save_dir,'/Time by quadrants/', graphtitle],'timeByQuad', '-ascii');
    % save([save_dir,'/Fraction of time by quadrants/', graphtitle],'timeByQuadFrac', '-ascii');


%% Calculate total distance per 5 minutes
INTERVAL_SIX_START = 60000;
INTERVAL_SIX_END = 71999;
...
if(abs(totalDistance - totaldistance_cm) > 0.01)
    ...
    % save([save_dir,'/Total distance per interval/', graphtitle],'distanceByintervals', '-ascii');
end
...
%daspect([1 0.01 1])
% saveas(gcf,[save_dir,'/Speed plots/',graphtitle,'.jpg']);
%timemoving = (totalspeed >= 3);

% save([save_dir,'/Speed over time/', graphtitle],'totalspeed', '-ascii');
% save([save_dir,'/Average speed/', graphtitle],'averagespeed', '-ascii');
